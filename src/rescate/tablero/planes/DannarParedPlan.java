...
class DannarParedPlan extends Plan {
  ...
  public void body() {
    ...
    // Si la conexión no es una pared o es una paerd ya rota...
    ...
    // Si la conexión es una pared que puede ser dannana..
    else {
      // No tiene PA suficientes en funcion de ...
      if ((jugador.getRol()!=Jugador.Rol.RESCATES && jugador.getPuntosAccion() < 2) || (jugador.getRol()==Jugador.Rol.RESCATES && jugador.getPuntosAccion() < 1))  {
      ...
      // PA suficientes...
        ...
        c.getConexiones()[accion.getConexion()] = Casilla.Conexion.values()[c.getConexiones()[accion.getConexion()].ordinal() + 1];
        ...
        switch (accion.getConexion()) {
          ...
          case 0:
            ...
            colindante.getConexiones()[2] = Casilla.Conexion.values()[colindante.getConexiones()[accion.getConexion()].ordinal() + 1];
            break;
          // Derecha
          case 1:
            colindante = t.getMapa()[c.getPosicion()[1]][c.getPosicion()[0] + 1];
            colindante.getConexiones()[3] = Casilla.Conexion.values()[colindante.getConexiones()[accion.getConexion()].ordinal() + 1];
            break;
          // Abajo
          case 2:
            colindante = t.getMapa()[c.getPosicion()[1] + 1][c.getPosicion()[0]];
            colindante.getConexiones()[0] = Casilla.Conexion.values()[colindante.getConexiones()[accion.getConexion()].ordinal() + 1];
            break;
          // Izquierda
          case 3:
            colindante = t.getMapa()[c.getPosicion()[1]][c.getPosicion()[0] - 1];
            colindante.getConexiones()[1] = Casilla.Conexion.values()[colindante.getConexiones()[accion.getConexion()].ordinal() + 1];
            break;
          // ...
          default:
            break;
        ...
        if(jugador.getRol()!=Jugador.Rol.RESCATES){
        ...
        respuesta.setContent(new ParedDannana());
        respuesta.getParameterSet(SFipa.RECEIVERS).addValue(idJugador);
        sendMessage(respuesta);
      }
		}
